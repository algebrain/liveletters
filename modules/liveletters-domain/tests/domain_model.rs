use liveletters_domain::{
    ...
    CommentHidden, CommentId, DomainError, EventId, Post, PostBody, PostCreated, PostId,
    ResourceId, Timestamp, Visibility,
};
...
fn comment_created_event_keeps_parent_comment_link() {
    let event = CommentCreated::new(CommentCreatedFields {
        ...
        comment_id: CommentId::new("comment-1").unwrap(),
        post_id: PostId::new("post-1").unwrap(),
        parent_comment_id: Some(CommentId::new("comment-root").unwrap()),
        resource_id: ResourceId::new("blog-1").unwrap(),
        actor_id: AccountId::new("alice").unwrap(),
        created_at: Timestamp::from_unix_seconds(1_710_000_100),
