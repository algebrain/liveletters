...
- хранение `CommandContext` (home + identity);
...
Контекст не хранит «текущего пользователя операционной системы» и не пытается определить, кто запустил процесс. `identity_name` приходит из файла `<home>/current-user` (см. `liveletters_config::read_current_identity`). Это сознательное решение: ни переменная окружения `LLTT_CU`, ни флаг `--as` сейчас **не поддерживаются** — иначе состояние «выбран через `lltt cu`, но `LLTT_CU` указывает на другое» становилось бы неотлаживаемым.
