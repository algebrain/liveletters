...
/// (`lltt sync`, `lltt sync pull` и `lltt sync push`). Этот модуль остался как
...
pub fn run(_ctx: &CommandContext, _args: &Args) -> Result<(), SyncError> {
    unimplemented!("сетевая синхронизация доступна через `lltt sync` (крейт liveletters-lltt-sync)")
}
