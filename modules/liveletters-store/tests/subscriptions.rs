...
fn save_subscription_round_trip() {
    let (_dir, store) = open();
    store
        .save_subscription(&record("alice-publish@example.org", "bob-feed@example.org"))
        .unwrap();
...
fn list_for_resource_filters_by_resource() {
    ...
    store
        ...
        .unwrap();
    store
        .save_subscription(&record("dave-publish@example.org", "bob-feed@example.org"))
        .unwrap();
    ...
    assert_eq!(
        dave_subs[0].subscriber_delivery_address,
