...
Для `visibility = friends_only` запись в `outbox` получает доставку
`ResourceFriends`: сетевой слой отправит её только подписчикам, которые
...
2. берёт видимость из исходного поста;
3. строит доменные идентификаторы и `CommentBody`;
4. создает доменную сущность `Comment`;
5. сохраняет материализованный `CommentRecord`;
6. строит событие `CommentCreated`;
7. строит `ProtocolMessage`;
8. ставит сообщение в outbox.

То есть команда не ограничивается созданием локального комментария, а сразу выражает его как исходящее доменное изменение.

У комментария нет самостоятельной входной видимости: он наследует
фактическую видимость исходного поста. Если автор комментирует собственную
закрытую запись, комментарий отправляется тому же кругу друзей-подписчиков.

## `FriendCommand`

### Зачем нужна эта команда

`FriendCommand` выражает намерение текущего пользователя добавить адрес в
друзья.
...
### Что реально делает use case `friend`
...
1. проверяет адрес владельца и адрес друга;
2. если подписка на друга уже есть, сохраняет `friends` и ставит в
   `outbox` событие `friend_added`;
3. если подписки ещё нет, сохраняет `pending_friends` и запускает обычный
   `subscribe`.

`SubscriptionRequested` при этом не получает специального назначения:
после успешного `SubscriptionConfirmed` прикладный слой сам завершает
ожидающее добавление в друзья и отправляет отдельный `friend_added`.
