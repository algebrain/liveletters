...
Текущий сценарий друзей работает так:

- `friend` сохраняет друга сразу, если подписка на него уже подтверждена;
- если подписки ещё нет, создаётся `pending_friends` и отправляется
  обычный запрос подписки;
- после входящего подтверждения подписки прикладный слой завершает
  добавление в друзья и ставит в исходящую очередь `friend_added`;
- записи и комментарии с видимостью `friends_only` отправляются только
  подписчикам, которые есть в списке друзей владельца ресурса.
