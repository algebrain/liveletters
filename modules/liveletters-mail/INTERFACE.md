...
- `ConfiguredSmtpTransport` (доступен только при включённой feature `network`)

Тестовая проверка отправки выполняется на уровне `liveletters-mime` через round-trip `build_protocol_email` + `decode_protocol_message`, а также через интеграционные тесты `tests/network_flow.rs`, которые поднимают локальный `TcpListener` и проверяют реальный TCP-обмен, не отходя от настоящей сетевой семантики.

## `ConfiguredSmtpTransport`

### Зачем нужен

Это реальный TCP-based SMTP seam текущей реализации, доступный под `#[cfg(feature = "network")]`.
...
- `ConfiguredImapMailbox` (доступен только при включённой feature `network`)

Тестовая проверка чтения выполняется на уровне `liveletters-mime` через разбор заранее подготовленных raw-писем функциями `parse_email` и `extract_liveletters_parts`, а также через интеграционные тесты `tests/network_flow.rs`, которые поднимают локальный `TcpListener` и проверяют реальный TCP-обмен.

## `ConfiguredImapMailbox`

### Зачем нужен

Это реальный TCP-based IMAP seam текущей реализации, доступный под `#[cfg(feature = "network")]`.
...
Сейчас retry-логика консервативная:
