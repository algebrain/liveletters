...
| `lltt user add <имя> [--from <путь>] [--yes\|-y]` | Читает TOML, проверяет имя, валидирует `mail.publish` (см. ниже), подтверждает пароли, скрывает их при `pwd_obfuscate = true`, сохраняет `<home>/identities/<имя>.toml` и копирует почтовые секции в `mail_settings`. Без `--from` берёт `<home>/drafts/<имя>.toml`. При `--yes` или `-y` не просит повторить пароли, а доверяет значениям из TOML. Текущего пользователя не меняет. |
...
того, как таблицы `resources_owned` и `pending_subscriptions` начнут на
...
`resources_owned` и `pending_subscriptions` указывают на существующие
записи в `authors`.

Адреса из `meta.subscriptions` не становятся подтверждёнными подписками
сразу. Для каждого внешнего адреса `user add` создаёт ожидающую
подписку и исходящее протокольное письмо `subscription_requested`.
Подписка появится в `local_subscriptions` только после входящего
подтверждения `subscription_confirmed`.
...
Без `--yes` / `-y` `lltt user add` просит повторить каждый непустой
SMTP- и IMAP-пароль. Ввод скрытый: на экране печатаются звёздочки.
Если подтверждение не совпало, команда завершается ошибкой и не
сохраняет изменённый пароль. `pwd_obfuscate` на это не влияет.

Пароль скрывается в исходном TOML только если выполнены все условия:

- пароль непустой;
- пароль ещё не начинается с `obf:v1:`;
- в соответствующей секции стоит `pwd_obfuscate = true`.

При `lltt user add --yes` или `lltt user add -y` повторный ввод не
выполняется: команда считает пароли из TOML уже подтверждёнными. После
этого скрытие всё равно зависит только от `pwd_obfuscate`.
