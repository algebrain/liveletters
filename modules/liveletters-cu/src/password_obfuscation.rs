...
pub trait PasswordConfirmer {
    fn confirm(&mut self, label: &'static str, password: &str) -> Result<(), CuError>;
}
...
impl PasswordConfirmer for DialoguerPasswordConfirmer {
    fn confirm(&mut self, label: &'static str, password: &str) -> Result<(), CuError> {
        let confirmation = read_masked_password(label)?;
...
pub fn confirm_identity_passwords(
    cfg: &IdentityConfig,
    confirmer: &mut dyn PasswordConfirmer,
) -> Result<(), CuError> {
    if let Some(smtp) = cfg.mail.smtp.as_ref()
        && should_confirm(&smtp.password)
    {
        confirmer.confirm("SMTP", &smtp.password)?;
    }

    if let Some(imap) = cfg.mail.imap.as_ref()
        && should_confirm(&imap.password)
    {
        confirmer.confirm("IMAP", &imap.password)?;
    }
...
fn should_confirm(password: &str) -> bool {
    !password.is_empty()
}
...
mod tests {
    ...
    impl PasswordConfirmer for FixedConfirmer {
        fn confirm(&mut self, label: &'static str, password: &str) -> Result<(), CuError> {
            let confirmation = self.values.pop_front().unwrap_or_default();
            if confirmation != password {
                return Err(CuError::PasswordConfirmationMismatch(label));
            }
            Ok(())
    ...
    fn obfuscates_smtp_and_imap_passwords_after_confirmation() {
        ...
        confirm_identity_passwords(&cfg, &mut confirmer).unwrap();
        let changed = obfuscate_identity_passwords(tmp.path(), &mut cfg).unwrap();
    ...
    fn confirmation_mismatch_errors_without_changing_passwords() {
        let cfg = identity_with_passwords("smtp-secret", "imap-secret", true);
        let mut confirmer = FixedConfirmer::new(&["wrong"]);

        let err = confirm_identity_passwords(&cfg, &mut confirmer).unwrap_err();
    ...
    #[test]
    fn confirmation_mismatch_errors_even_when_obfuscation_disabled() {
        let cfg = identity_with_passwords("smtp-secret", "imap-secret", false);
        let mut confirmer = FixedConfirmer::new(&["wrong"]);
