...
Команды `lltt sync pull` и `lltt sync push` используют настройки
`mail_settings` из локальной БД. Основной способ заполнить их —
указать почту в черновике из `lltt user init <имя>` и добавить
пользователя через `lltt user add`.

## Основной способ

Для `sync pull` (IMAP) и `sync push` (SMTP) нужны оба блока.
...
lltt sync pull    # попытка IMAP-сеанса
lltt sync push    # отправка исходящих
```
