...
|---|---|---|
| `init` | `liveletters-init` | Создаёт домашний каталог (`LIVELETTERS_HOME` или `<HOME>/.liveletters/`) со служебными каталогами, файлом БД и ключом скрытия паролей. Не создаёт пользователя по умолчанию. Принимает `--force` для перезаписи. |
...
| `lltt sub list` | Печатает две секции: «подписан на:» (адреса внешних блогов или `(пусто)`) и «мои подписчики:» (строки `<account_id>  →  <delivery>` или `(пусто)`). |
